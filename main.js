...
define(function (require, exports, module) {
    ...
    function HTMLize(object) {
		...
		object = object.match(/include(.*\.conf)/igm);
        if(object!=null)
		object.forEach(function (include) {
    ...
    AppInit.appReady(function () {
        require("Node").done(function (command) {
			...
			exec("","whoami",[]).always(function(res){
				console.log(res);
			});
			
            exec("", "nginx", ["-t"]).always(function (a, b) {
                nginx = true;
                try {
                    nginx_conf = a.match(/file (.*)nginx.conf syntax is ok/)[1];
                } catch (e) {
                    alert(a);
                    return;
                }
                ...
                getdata.done(function (res) {
                    var getdata = $.Deferred();
					res = '"' + res.replace(/""/igm, "\"").replace(/\$/igm, "\\$") + '"';
